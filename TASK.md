...
WebSearchで以下6クエリを検索し、各クエリから8件ずつ、計48件を収集する。

1. `半導体 市場 {year}年{month}月`
2. `AI 半導体 投資 {year}年{month}月`
3. `半導体 業績 決算 {year}年{month}月`
4. `電子部品 半導体 {year}年{month}月`
5. `半導体 製造装置 設備投資 {year}年{month}月`
6. `半導体 検査装置 {year}年{month}月`

**日付フィルタ**: 対象月（`{year}-{month:02d}`）に公開された記事のみを含める。対象月外・日付不明・重複は除外する。
